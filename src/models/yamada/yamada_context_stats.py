# Yamada model that uses context information as well as corpus stats (priors and conditionals)
import torch
...
class YamadaContextStats(YamadaBase):

    def __init__(self, yamada_model=None, args=None):
        super().__init__(yamada_model, args)

        self.hidden = nn.Linear(3 + 2 * self.emb_dim, self.args.hidden_size)
        self.output = nn.Linear(self.args.hidden_size, 1)

    def forward(self, inputs):
        tokens, candidate_ids, priors, conditionals = inputs
        ...
        input = torch.cat((token_embs, dot_product, candidate_embs, priors, conditionals), dim=3)
