...
class SmallContext(CombinedBase):
    ...
    def forward(self, inputs):
        ...
        if len(mention_word_tokens.shape) == 3:
            ...
            candidate_repr = torch.cat((candidate_mention_embs, candidate_embs), dim=2)

            # Normalize
            if self.args.norm_final:
                candidate_repr = F.normalize(candidate_repr, dim=2)
                mention_repr = F.normalize(mention_repr, dim=1)

            mention_repr.unsqueeze_(1)

            # Dot product over last dimension
            scores = (mention_repr * candidate_repr).sum(dim=2)
            ...
            # Cat the embs
            mention_repr = torch.cat((mention_embs_agg, context_embs_agg), dim=1)
            candidate_repr = torch.cat((candidate_mention_embs, candidate_embs), dim=1)
            ...
            return candidate_repr, mention_repr
