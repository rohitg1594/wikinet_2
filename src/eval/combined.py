...
class CombinedValidator:
    ...
    def _numpify_data(self, data_type='wiki'):
        ...
        if data_type == 'wiki':
            out_f = join(self.args.data_path, 'cache',
                         f'eval_mention_tokens_{data_type}_{self.args.data_type}_{self.args.train_size}')
        else:
    ...
    def validate(self, model=None, error=True):
        ...
        if self.args.model_name == 'full_context_string_scalar':
            self.run_combination_weight_exp(model, results)
        else:
            for data_type in self.data_types:
                ...
                result = eval_ranking(preds, gold, [1, 2, 5, 10, 100])
                results[data_type] = result
                logger.info(f'DATA TYPE : {data_type}, RESULTS : {result}')
                ...
                if error:
                    ...
                    check_errors(preds, gold, mention_gram, self.id2ent, self.id2gram, self.redirects,
                                 [1, 10, 100])
                    print()
    ...
    def run_combination_weight_exp(self, model, results):
        ...
        for context_w in weights:
            for str_w in weights:
                ...
                combination_key = context_w, str_w, prior_w
                combination_grid_results[combination_key] = {}
